...
export function SPDocumentsPage() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div>
        <h1 style={{ fontSize: 24, fontWeight: 800, color: '#1A365E', margin: 0 }}>My Documents</h1>
        <p style={{ fontSize: 16, color: '#7A92B0', margin: '4px 0 0' }}>Required enrollment documents and submission status</p>
      </div>

      <div style={{ ...card, display: 'flex', alignItems: 'center', gap: 16 }}>
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
            <span style={{ fontSize: 16, fontWeight: 600, color: '#1A365E' }}>Documents Submitted</span>
            <span style={{ fontSize: 16, fontWeight: 700, color: submitted === REQUIRED_DOCS.length ? '#10B981' : '#F59E0B' }}>{submitted}/{REQUIRED_DOCS.length}</span>
          </div>
        ...
        {REQUIRED_DOCS.map(docName => {
          ...
          return (
            <div key={docName} style={{ ...card, padding: '12px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: isOk ? '#F0FBF5' : '#FFF8F8', border: `1px solid ${isOk ? '#A7E3C0' : '#F5C2C7'}` }}>
              <div>
                <div style={{ fontSize: 16, fontWeight: 600, color: '#1A365E' }}>{docName}</div>
                {doc?.uploaded_at && <div style={{ fontSize: 15, color: '#7A92B0' }}>Uploaded {new Date(doc.uploaded_at).toLocaleDateString()}</div>}
                {doc?.notes && <div style={{ fontSize: 15, color: '#7A92B0' }}>{doc.notes}</div>}
              </div>
              <span style={{ padding: '3px 10px', borderRadius: 6, fontSize: 15, fontWeight: 700, background: isOk ? '#E8FBF0' : '#FEE2E2', color: isOk ? '#0E6B3B' : '#991B1B', flexShrink: 0 }}>
                {status}
              </span>
            </div>
          )
      ...
      {docs.filter(d => !REQUIRED_DOCS.includes(d.document_name)).map(d => (
        <div key={d.id} style={{ ...card, padding: '12px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ fontSize: 16, fontWeight: 600, color: '#1A365E' }}>{d.document_name}</div>
          <span style={{ padding: '3px 10px', borderRadius: 6, fontSize: 15, fontWeight: 700, background: '#E6F4FF', color: '#0369A1' }}>{d.status}</span>
        </div>
      ))}
    </div>
  )
