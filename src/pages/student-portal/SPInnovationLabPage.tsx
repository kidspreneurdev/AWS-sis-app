    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(10,24,50,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
      <div style={{ background: '#fff', borderRadius: 14, width: '100%', maxWidth: 500, boxShadow: '0 20px 60px rgba(0,0,0,0.25)', overflow: 'hidden' }}>
        <div style={{ background: 'linear-gradient(135deg,#0F2240,#1A365E)', padding: '16px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ fontSize: 18, fontWeight: 700, color: '#fff' }}>New Lab Project</div>
        ...
        <div style={{ padding: '12px 20px', borderTop: '1px solid #E4EAF2', display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
          ...
          <button onClick={handleSave} disabled={saving} style={{ padding: '8px 20px', borderRadius: 8, border: 'none', background: '#D61F31', color: '#fff', fontWeight: 600, fontSize: 16, cursor: 'pointer' }}>{saving ? 'Saving…' : 'Add'}</button>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <div><h1 style={{ fontSize: 24, fontWeight: 800, color: '#1A365E', margin: 0 }}>Innovation Lab</h1><p style={{ fontSize: 16, color: '#7A92B0', margin: '4px 0 0' }}>Track your innovation and prototype projects</p></div>
        {!readOnly && <button onClick={() => setModal(true)} style={{ padding: '9px 18px', borderRadius: 8, border: 'none', background: '#D61F31', color: '#fff', fontWeight: 600, fontSize: 16, cursor: 'pointer' }}>+ New Project</button>}
      </div>
          ...
          return (
            <div key={stage} style={{ background: '#F7F9FC', borderRadius: 10, padding: 12 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
                <span style={{ fontSize: 15, fontWeight: 700, color: sc.tc, textTransform: 'uppercase' }}>{stage}</span>
                <span style={{ fontSize: 15, background: sc.bg, color: sc.tc, borderRadius: 4, padding: '1px 6px', fontWeight: 700 }}>{stageProjects.length}</span>
              </div>
              {stageProjects.map(p => (
      ...
      {projects.length === 0 && (
        <div style={{ textAlign: 'center', padding: 40, color: '#7A92B0', fontSize: 16, background: '#fff', borderRadius: 12, border: '1px solid #E4EAF2' }}>
