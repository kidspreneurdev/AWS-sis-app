...
import { Gem, Medal, Award, Star, Lock, Sparkles, type LucideIcon } from 'lucide-react'
...
function starLevel(count: number): { label: string; color: string; icon: LucideIcon } {
  if (count >= 20) return { label: 'Platinum', color: '#7C3AED', icon: Gem }
  if (count >= 10) return { label: 'Gold', color: '#FAC600', icon: Award }
  if (count >= 5)  return { label: 'Silver', color: '#94A3B8', icon: Medal }
  return { label: 'Bronze', color: '#B45309', icon: Medal }
}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      {badges.length < 20 && (
        <div style={{ background: '#fff', border: '1.5px solid #E2E8F0', borderRadius: 14, padding: 16 }}>
          {(() => {
            const next = badges.length < 5 ? { need: 5, label: 'Silver', Icon: Medal }
              : badges.length < 10 ? { need: 10, label: 'Gold', Icon: Award }
              : { need: 20, label: 'Platinum', Icon: Gem }
